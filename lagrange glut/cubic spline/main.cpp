/*
    ...
    Zulker Nayeen
    Roll :FH- 11
    this is the code to plot graph using n+1 points,
    method used : cubic spline

    given
    X coordinates : x[]
    Y coordinates : a[]

    //cubic spline : Sj(x) = aj + bj(x − xj) + cj(x − xj)^2 + dj(x − xj)^3,
...
static int Ym = 1000, Xm = 1500;

float  c[n], b[n], d[n];
float x[n+1] = {-350,-175,0,175,350}; // points on X
float a[n+1] = {-300,-72.67,62.7,217.33,403.67};  // points on Y
...
static void display(void)
{
    ...
    //plotting the given coordinate points
    glPointSize(5);
    glBegin(GL_POINTS);
